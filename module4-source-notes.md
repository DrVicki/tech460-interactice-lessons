...
## Animated trace enhancement — September 25, 2026

The hash-map and two-pointer traces now use independent, reusable `Module4AnimatedTrace` components and deterministic micro-step models in `module4Animation.ts`. Hash-map states distinguish input reading, default/existing-key lookup, and dictionary insertion/update. Two-pointer states distinguish comparison, the sorted-order elimination argument, and the actual pointer move; found and exhausted outcomes terminate cleanly. Input focus, traveling value tokens, dictionary entries/counts, pointer rails, and ruled-out candidates use transform/opacity animations lasting 280 ms. De-emphasized text remains readable, and labels do not depend on color alone.

Each lab has explicit Play/Pause, three playback paces, Previous/Next, a step timeline, reset, scenario selection, and optional highlighted Python logic. There is no automatic start. Playback pauses on tab hiding or when the lab leaves view, and pending timers clean up on state changes or unmount. Reduced-motion settings use instant transitions without losing algorithm states; keyboard-initiated stepping is also instant. The visuals illustrate fixed companion inputs and do not execute external editor code or affect workbook completion.

Standalone views at `/labs/hash-maps` and `/labs/two-pointers` reuse exactly the same components without the course sidebar or progress controls. Each lesson exposes a Standalone lab link, and standalone pages provide clear return navigation and the Canvas access reminder. These views support reuse separately from the lessons; no automatic Canvas/LMS integration, SCORM reporting, or GitHub Pages update was performed.

Validation: 86 unit/Python tests pass (the existing 75 plus 11 micro-step and intermediate-state tests), TypeScript passes, and the production build passes with the existing non-blocking font-import-order and bundle-size warnings. Chromium testing verifies insertion versus update, backward stepping, actual intermediate pointer transforms, comparison/decision/move sequencing, duplicates/no-match/empty scenarios, pace changes, finite playback, replay, keyboard controls, visibility pause, reduced-motion preference changes, and standalone navigation. Both standalone and lesson views fit 320px and 390px widths; desktop/mobile screenshots were inspected. Existing Module 3 and Module 4 learner-flow regression suites also pass. The standalone public preview returned HTTP 200.

The previously failing hidden-tab assertion was a test synchronization issue: it read state before React applied the event-driven update. The final test waits for the pause state, then verifies it. Remote embedded Python execution remains outside this test scope.
