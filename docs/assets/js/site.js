...
function loadProgress() {
  const saved = localStorage.getItem('tech460-static-progress');
  if (saved) {
...
function saveProgress() {
  localStorage.setItem('tech460-static-progress', JSON.stringify([...completedModules]));
}
...
function updateProgressSummary() {
  ...
  if (progressText) {
    progressText.textContent = `${completed} of ${total} modules completed (${percentage}%)`;
  }
}
...
function renderCurriculum() {
  ...
  container.innerHTML = courseModules.map(module => `
    <div class="curriculum-item ${completedModules.has(module.id) ? 'completed' : ''}" onclick="toggleModule('${module.id}')">
      <div class="curriculum-checkbox">
